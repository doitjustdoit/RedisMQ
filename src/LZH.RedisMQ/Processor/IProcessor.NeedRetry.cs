...
public class MessageNeedToRetryProcessor : IProcessor
{
    ...
    public MessageNeedToRetryProcessor(IOptions<RedisMQOptions> options, ILogger<MessageNeedToRetryProcessor> logger, IDispatcher dispatcher)
    {
        _logger = logger;
    ...
    private async Task ProcessPublishedAsync(ProcessingContext context)
    {
    ...
    private async Task ProcessReceivedAsync(ProcessingContext context)
    {
        context.ThrowIfStopping();
